         ...
         <div class="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-xl rounded-lg -mt-80">
           ...
           <div>
               ...
               <div class="w-full lg:w-3/12 lg:order-2 flex justify-center">
                 <div class="relative">
                   ...
                   <div class="mt-2" x-show="! photoPreview">
                     <img src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}" class="shadow-xl rounded-full h-auto align-middle border-none absolute -m-16 -ml-20 lg:-ml-16" style="min-width: 150px;">
                   </div>
                 </div>
               </div>
             ...
             <div class="text-center mt-8">
               <h3 class="text-4xl font-semibold leading-normal mb-1 text-gray-800 mb-1">
                 {{$user->name}}
               </h3>
               <div>
                @if($user->feedbackTo->count() != 0)
                 @for($i=1 ; $i <= round($stars_count/$user->feedbackTo->count()) ; $i++)
                   ...
                   @for($i=round($stars_count/$user->feedbackTo->count())+1 ; $i <=5 ; $i++) 
                   <i aria-hidden="true" class="fas fa-star text-gray-300"></i>
                     @endfor
                  @else
                  @for($i=1 ; $i <=5 ; $i++) 
                  <i aria-hidden="true" class="fas fa-star text-gray-300"></i>
